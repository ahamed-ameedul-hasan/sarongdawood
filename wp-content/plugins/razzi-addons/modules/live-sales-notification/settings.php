...
class Settings  {
	...
	public function live_sales_notification_section( $sections ) {
		$sections['razzi_live_sales_notification'] = esc_html__( 'Live Sales Notification', 'razzi-addons' );
	...
	public function live_sales_notification_settings( $settings, $section ) {
		if ( 'razzi_live_sales_notification' == $section ) {
			...
			$settings[] = array(
				...
				'title' => esc_html__( 'Live Sales Notification', 'razzi-addons' ),
				'type'  => 'title',
			);
			...
			$settings[] = array(
				...
				'title'   => esc_html__( 'Live Sales Notification', 'razzi-addons' ),
				'desc'    => esc_html__( 'Enable Live Sales Notification', 'razzi-addons' ),
				'type'    => 'checkbox',
				'default' => 'no',
			);

			$settings[] = array(
				'name'    => esc_html__( 'Select product from', 'razzi-addons' ),
				'id'      => 'razzi_live_sales_notification_navigation',
				'default' => 'product-type',
				'class'   => 'razzi_live_sales_notification_navigation wc-enhanced-select',
				'type'    => 'select',
				'options' => array(
					'orders'                   => esc_html__( 'Orders placed', 'razzi-addons' ),
					'product-type' 			   => esc_html__( 'Product type', 'razzi-addons' ),
					'selected-products'        => esc_html__( 'Show selected products', 'razzi-addons' ),
					'selected-categories'      => esc_html__( 'Show product from selected category', 'razzi-addons' ),
				),
			);

			$settings[] = array(
				'name'    => esc_html__( 'Select product type', 'razzi-addons' ),
				'id'      => 'razzi_live_sales_notification_product_type',
				'default' => 'recent',
				'class'   => 'product-type-show wc-enhanced-select live-sales--condition',
				'type'    => 'select',
				'options' => array(
					'recent'       => esc_html__( 'Recently viewed', 'razzi-addons' ),
			...
			$settings[] = array(
				'name'    => esc_html__( 'Based on the order status orders will be selected', 'razzi-addons' ),
				'id'      => 'razzi_live_sales_notification_order',
				'default' => 'wc-completed',
				'class'   => 'wc-enhanced-select orders-show live-sales--condition',
				'type'    => 'multiselect',
				'options' => array(
					'wc-pending'        => esc_html__( 'Pending payment', 'razzi-addons' ),
					'wc-processing'     => esc_html__( 'Processing', 'razzi-addons' ),
					'wc-on-hold'        => esc_html__( 'On hold', 'razzi-addons' ),
					'wc-completed'      => esc_html__( 'Completed', 'razzi-addons' ),
					'wc-cancelled'      => esc_html__( 'Cancelled', 'razzi-addons' ),
					'wc-refunded'       => esc_html__( 'Refunded', 'razzi-addons' ),
					'wc-failed'         => esc_html__( 'Failed', 'razzi-addons' ),
					'wc-checkout-draft' => esc_html__( 'Draft', 'razzi-addons' ),
				),
			);

			$settings[] = array(
				'name'    => esc_html__( 'Select product', 'razzi-addons' ),
			...
			$settings[] = array(
				'name'    => esc_html__( 'Select category', 'razzi-addons' ),
			...
			$settings[] = array(
				'title'       => __( 'Virtual first name', 'razzi-addons' ),
				...
				'desc_tip'    => __( 'This name will be used, when you decide to show virtual sales, Enter one name on one line', 'razzi-addons' ),
			);

			$settings[] = array(
				'title'       => __( 'Virtual location', 'razzi-addons' ),
				...
				e.g: city , , country, if you dont have city then e.g: , state, country', 'razzi-addons' ),
			);

			$settings[] = array(
				'name'    => esc_html__( 'Show orders placed in last', 'razzi-addons' ),
				...
				'default' => '1',
				'desc_tip' => esc_html__( 'E.g: 1 day: will show order placed in last one day', 'razzi-addons' ),
			);
			...
			$settings[] = array(
				...
				'type'    => 'select',
				'options' => array(
					'hour' => esc_html__( 'Hour', 'razzi-addons' ),
					'day'  => esc_html__( 'Day', 'razzi-addons' ),
					'week' => esc_html__( 'Week', 'razzi-addons' ),
				),
			);

			$settings[] = array(
				'title'   => __( 'Time passed type', 'razzi-addons' ),
				'id'      => 'razzi_live_sales_notification_time_passed_type',
				'default' => 'minutes',
				'class'   => 'wc-enhanced-select',
				'type'    => 'select',
				'options' => array(
					'seconds' => esc_html__( 'Seconds', 'razzi-addons' ),
					'minutes' => esc_html__( 'Minutes', 'razzi-addons' ),
					'hours'   => esc_html__( 'Hours', 'razzi-addons' ),
				),
			);

			$settings[] = array(
				'name'    => esc_html__( 'How many notification to show (make sure number is grater then 1)', 'razzi-addons' ),
				...
				'default' => '10',
				'desc_tip' => esc_html__( 'For virtual orders this many notification will be created, but for original orders if it is less then this number no virtual order will be created', 'razzi-addons' ),
			);

			$settings[] = array(
				'name'    => esc_html__( 'When to start showing popup (milliseconds)', 'razzi-addons' ),
				...
				'default' => '6000',
				'desc_tip' => esc_html__( 'Once a person comes to page, when to start showing popup', 'razzi-addons' ),
			);

			$settings[] = array(
				'name'    => esc_html__( 'How long to keep the popup opened (milliseconds)', 'razzi-addons' ),
				...
				'default' => '6000',
				'desc_tip' => esc_html__( 'How long to keep the popup open', 'razzi-addons' ),
			);

			$settings[] = array(
				'name'    => esc_html__( 'Time gap between showing of 2 popups (milliseconds)', 'razzi-addons' ),
				...
				'default' => '6000',
				'desc_tip' => esc_html__( 'Once a popup closes then after how much time new popup should open', 'razzi-addons' ),
			);
