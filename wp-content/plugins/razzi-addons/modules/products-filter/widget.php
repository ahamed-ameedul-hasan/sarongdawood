...
class Widget extends \WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			esc_html__( 'Razzi - Products Filter', 'razzi-addons' ),
			array(
				'classname'                   => 'products-filter-widget woocommerce',
				'description'                 => esc_html__( 'WooCommerce products filter.', 'razzi-addons' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['filter'] ) ) {
			...
			echo '<button type="submit" value="' . esc_attr__( 'Filter', 'razzi-addons' ) . '" class="button filter-button">' . esc_html__( 'Filter', 'razzi-addons' ) . '</button>';
			echo '<button type="reset" value="' . esc_attr__( 'Reset Filter', 'razzi-addons' ) . '" class="button alt reset-button button-lg">' . esc_html__( 'Reset Filter', 'razzi-addons' ) . '</button>';
			echo '</div>';
		...
		foreach ( $active_filters as $filter ) {
			...
			foreach ( $terms as $term ) {
				switch ( $filter['source'] ) {
					...
					case 'rating':
						$text = _n( 'Rated %d star', 'Rated %d stars', $term, 'razzi-addons' );
	...
	public function display_filter( $filter ) {
		...
		if ( 'attribute' == $filter['source'] ) {
			...
			$args['all']        = sprintf( esc_html__( 'Any %s', 'razzi-addons' ), wc_attribute_label( $attr->attribute_label ) );
			...
			$args['all'] = sprintf( esc_html__( 'Select a %s', 'razzi-addons' ), $taxonomy->labels->singular_name );
		} else {
			$args['all'] = esc_html__( 'All Products', 'razzi-addons' );
		}
        ...
        <div class="<?php echo esc_attr( join( ' ', $classes ) ) ?>">
            ...
            <div class="products-filter__filter-control filter-control">
                ...
                <div class="products-filter__control-buttons">
                    <button type="button" value="<?php esc_attr_e( 'Clear', 'razzi-addons' ); ?>"
                            class="button alt clear-button"><?php esc_html_e( 'Clear', 'razzi-addons' ); ?></button>
                    <button type="submit" value="<?php esc_attr_e( 'Show Results', 'razzi-addons' ) ?>"
                            class="button filter-button button-lg"><?php esc_html_e( 'Show Results', 'razzi-addons' ); ?></button>
                </div>
            </div>
        </div>
		...
		switch ( $filter['source'] ) {
			...
			case 'products_group':
				$filter_groups = apply_filters( 'razzi_products_filter_groups', array(
					'best_sellers' => esc_html__( 'Best Sellers', 'razzi-addons' ),
					'new'          => esc_html__( 'New Products', 'razzi-addons' ),
					'sale'         => esc_html__( 'Sale Products', 'razzi-addons' ),
					'featured'     => esc_html__( 'Hot Products', 'razzi-addons' ),
				) );

				if ( 'dropdown' != $filter['display'] ) {
					$options[''] = array(
						'name'  => esc_html__( 'All Products', 'razzi-addons' ),
						'count' => 0,
						'id'    => 0,
						'level' => 0,
					);
			...
			case 'product_status':
				if(! isset($filter['disable_onsale'])) {
					$options[ 'on_sale' ] = array(
						'name'  => esc_html__( 'On Sale', 'razzi-addons' ),
						'count' => 0,
						'id'    => 'on_sale',
						'level' => 0,
					);
				...
				if(! isset($filter['disable_in_stock'])) {
					$options[ 'in_stock' ] = array(
						'name'  => esc_html__( 'In Stock', 'razzi-addons' ),
						'count' => 0,
						'id'    => 'in_stock',
						'level' => 0,
					);
	...
	protected function filter_search_box( $filter ) {
		if ( 'attribute' == $filter['source'] ) {
			...
			$placeholder = __( 'Search', 'razzi-addons' ) . ' ' . strtolower( $attributes[ $filter['attribute'] ] );
		} else {
			$sources     = $this->get_filter_source_options();
			$placeholder = __( 'Search', 'razzi-addons' ) . ' ' . strtolower( $sources[ $filter['source'] ] );
		}
	...
	protected function display_dropdown( $args ) {
		$args = wp_parse_args( $args, array(
			...
			'current'     => array(),
			'options'     => array(),
			'all'         => esc_html__( 'Any', 'razzi-addons' ),
			'show_counts' => false,
		) );
	...
	public function form( $instance ) {
        ...
        <div class="razzi-products-filter-form__sub-nav">
            <button type="button" data-section="filters"
                    class="button-link active"><?php esc_html_e( 'Filters', 'razzi-addons' ); ?></button>
            |
            <button type="button" data-section="options"
                    class="button-link"><?php esc_html_e( 'Options', 'razzi-addons' ); ?></button>
        </div>
        ...
        <div class="razzi-products-filter-form__section active" data-section="filters">
            <p class="razzi-products-filter-form__message <?php echo ! empty( $instance['filter'] ) ? 'hidden' : '' ?>"><?php esc_html_e( 'There is no filter yet.', 'razzi-addons' ) ?></p>
            ...
            <p class="razzi-products-filter-form__section-actions">
                <button type="button" class="razzi-products-filter-form__add-new button-link"
                    ...
                    + <?php esc_html_e( 'Add a new filter', 'razzi-addons' ) ?></button>
            </p>
        </div>
        ...
        <div class="razzi-products-filter-form__section" data-section="options">
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'ajax',
				'label' => esc_html__( 'Use ajax for filtering', 'razzi-addons' ),
				'value' => $instance['ajax'],
			) );
			...
			$this->setting_field( array(
				'type'      => 'checkbox',
				'name'      => 'instant',
				'label'     => esc_html__( 'Filtering products instantly (no buttons required)', 'razzi-addons' ),
			...
			$this->setting_field( array(
				'type'      => 'select',
				'name'      => 'filter_buttons',
				'label'     => esc_html__( 'Filter buttons position', 'razzi-addons' ),
				'value'     => $instance['filter_buttons'],
				'options'   => array(
					'form'   => __( 'Bottom of form', 'razzi-addons' ),
					'fitems' => __( 'Bottom of filter items', 'razzi-addons' ),
				),
				'condition' => array(
					'instant' => false,
				),
			...
			$this->setting_field( array(
				'type'      => 'checkbox',
				'name'      => 'change_url',
				'label'     => esc_html__( 'Update URL', 'razzi-addons' ),
                ...
                <div class="razzi-products-filter-form__filter-top">
                    <div class="razzi-products-filter-form__filter-actions">
                        <button type="button"
                            ...
                            <span class="screen-reader-text"><?php esc_html_e( 'Remove filter', 'razzi-addons' ) ?></span>
	...
	protected function get_filter_source_options() {
		$sources = array(
			'orderby'        => esc_html__( 'Order By', 'razzi-addons' ),
			'products_group' => esc_html__( 'Group', 'razzi-addons' ),
			'price'          => esc_html__( 'Price', 'razzi-addons' ),
			'attribute'      => esc_html__( 'Attributes', 'razzi-addons' ),
			'rating'         => esc_html__( 'Rating', 'razzi-addons' ),
			'product_status' => esc_html__( 'Product Status', 'razzi-addons' ),
		);
	...
	protected function get_filter_display_options( $source = 'product_cat' ) {
		$options = array(
			'price'     => array(
				'slider' => esc_html__( 'Slider', 'razzi-addons' ),
				'ranges' => esc_html__( 'Ranges', 'razzi-addons' ),
			),
			'attribute' => array(
				'auto'       => esc_html__( 'Auto', 'razzi-addons' ),
				'dropdown'   => esc_html__( 'Dropdown', 'razzi-addons' ),
				'list'       => esc_html__( 'Vertical List', 'razzi-addons' ),
				'h-list'     => esc_html__( 'Horizontal List', 'razzi-addons' ),
				'checkboxes' => esc_html__( 'Checkbox List', 'razzi-addons' ),
			),
			'rating'    => array(
				'dropdown'   => esc_html__( 'Dropdown', 'razzi-addons' ),
				'checkboxes' => esc_html__( 'Checkbox List', 'razzi-addons' ),
	...
	protected function get_filter_fields_settings() {
		...
		$this->filter_settings = array(
			'name'        => array(
				'type'  => 'text',
				'label' => __( 'Filter Name', 'razzi-addons' ),
			),
			'source'      => array(
				'type'    => 'select',
				'label'   => __( 'Filter By', 'razzi-addons' ),
				'options' => $this->get_filter_source_options(),
			),
			'attribute'   => array(
				'type'      => 'select',
				'name'      => 'attribute',
				'label'     => __( 'Attribute', 'razzi-addons' ),
			...
			'display'     => array(
				'type'    => 'select',
				'label'   => __( 'Display Type', 'razzi-addons' ),
				'options' => $this->get_filter_display_options(),
			),
			'ranges'      => array(
				'type'      => 'textarea',
				'label'     => __( 'Ranges', 'razzi-addons' ),
				'desc'      => __( 'Each range on a line, separate by the <code>-</code> symbol. Do not include the currency symbol.', 'razzi-addons' ),
				'condition' => array(
					'display' => 'ranges',
					'source'  => 'price',
				),
			...
			'multiple'    => array(
				'type'      => 'select',
				'label'     => __( 'Selection Type', 'razzi-addons' ),
				'options'   => array(
					0 => __( 'Single select', 'razzi-addons' ),
					1 => __( 'Multiple select', 'razzi-addons' ),
				),
				'condition' => array(
			...
			'query_type'  => array(
				'type'      => 'select',
				'label'     => __( 'Query Type', 'razzi-addons' ),
				'options'   => array(
					'and' => __( 'AND', 'razzi-addons' ),
					'or'  => __( 'OR', 'razzi-addons' ),
				),
				'condition' => array(
					'source' => 'attribute',
				),
			...
			'collapsible' => array(
				'type'      => 'checkbox',
				'label'     => __( 'Collapsible', 'razzi-addons' ),
				'condition' => array(
			...
			'show_counts' => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show product counts', 'razzi-addons' ),
				'condition' => array(
			...
			'searchable'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show the search box', 'razzi-addons' ),
				'condition' => array(
			...
			'scrollable'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Limit the height of items list (scrollable)', 'razzi-addons' ),
				'condition' => array(
					'source!'  => array( 'product_status' ),
			...
			'show_empty_cats'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show empty categories', 'razzi-addons' ),
				'condition' => array(
					'source'  => array( 'product_cat' ),
				),
			),
			'disable_onsale'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable on sale', 'razzi-addons' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
			'disable_in_stock'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable in stock', 'razzi-addons' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
