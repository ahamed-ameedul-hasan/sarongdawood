...
class Settings  {
	...
	public function product_tabs_section( $sections ) {
		$sections['razzi_addons_product_tabs'] = esc_html__( 'Product Tabs', 'razzi-addons' );
	...
	public function product_tabs_settings( $settings, $section ) {
		...
		$settings_product_tabs = array(
			array(
				'name' => esc_html__( 'Product Tabs', 'razzi-addons' ),
				'type' => 'title',
				'id'   => self::OPTION_NAME . '_options',
			),
			array(
				'name'    => esc_html__( 'Product Tabs', 'razzi-addons' ),
				'desc'    => esc_html__( 'Enable product tabs manager', 'razzi-addons' ),
				'id'      => self::OPTION_NAME,
				'default' => 'no',
				'type'    => 'checkbox',
				'checkboxgroup' => 'start',
	...
	public function edit_admin_columns( $columns ) {
		$columns = array_merge( $columns, array(
			'product_tab_disable' => esc_html__( 'Disabled', 'razzi-addons' ),
			'product_tab_type' => esc_html__( 'Type', 'razzi-addons' ),
			'product_tab_products' => esc_html__( 'Products', 'razzi-addons' ),
			'product_tab_categories' => esc_html__( 'Categories', 'razzi-addons' ),
		) );
	...
	public function manage_custom_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'product_tab_disable':
				...
				echo $tab_disable == 'yes' ? esc_html__( 'Yes', 'razzi-addons' ) : esc_html__( 'No', 'razzi-addons' );
			...
			case 'product_tab_type':
				...
				if ( ! is_wp_error( $types ) && $types && is_array( $types ) ) {
					...
					switch( $type_name ) {
						case 'global':
							esc_html_e('Global', 'razzi-addons') ;
							break;
						case 'product':
							esc_html_e('Product', 'razzi-addons') ;
							break;
						case 'custom':
							esc_html_e('Custom', 'razzi-addons') ;
							break;
						default:
							esc_html_e('Default', 'razzi-addons') ;
							break;
					}
				} else {
					esc_html_e( 'No Type', 'razzi-addons' );
				}
			...
			case 'product_tab_products':
				...
				if (  $product_ids ) {
					...
					$links[] = esc_html__( 'No Product', 'razzi-addons' );
				}
			...
			case 'product_tab_categories':
				...
				if ( ! is_wp_error( $cats ) && $cats && is_array( $cats ) ) {
					...
					$links[] = esc_html__( 'No Category', 'razzi-addons' );
				}
	...
	public function meta_boxes( $post ) {
		add_meta_box( 'razzi-product-tabs', esc_html__( 'Tabs Settings', 'razzi-addons' ), array( $this, 'tabs_meta_box' ), self::POST_TYPE, 'advanced', 'high' );
	}
	...
	public function tabs_meta_box( $post ) {
		...
		<div id="razzi-product-tabs-settings" class="razzi-product-tabs-settings">
			<p class="form-field">
				<label><?php esc_html_e('Tab Type', 'razzi-addons'); ?></label>
				<span class="razzi-product-tabs--type">
					...
					<label for="razzi-product-tab-global"><?php esc_html_e('Global', 'razzi-addons'); ?></label>
					...
					<label for="razzi-product-tab-product"><?php esc_html_e('Product', 'razzi-addons'); ?></label>
					...
					<label for="razzi-product-tab-custom"><?php esc_html_e('Custom', 'razzi-addons'); ?></label>
				</span>

			</p>
			<p class="form-field razzi-product-tabs--product">
				<label for="product-tab-id"><?php esc_html_e( 'Products', 'razzi-addons' ); ?></label>
				<select class="wc-product-search" multiple="multiple" style="width: 50%;" id="product-tab-id" name="_product_tab_product_ids[]" data-sortable="true" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'razzi-addons' ); ?>" data-action="woocommerce_json_search_products">
					<?php
			...
			<p class="form-field razzi-product-tabs--categories">
				<label for="product_tab_categories"><?php esc_html_e( 'Categories', 'razzi-addons' ); ?></label>
				<select class="wc-category-search" multiple="multiple" style="width: 50%;" id="product_tab_categories" name="_product_tab_cat_slugs[]" data-sortable="true" data-placeholder="<?php esc_attr_e( 'Search for a category&hellip;', 'razzi-addons' ); ?>" data-action="woocommerce_json_search_categories">
					<?php
	...
	public function create_post_type() {
		$posts = array(
			'description' 				=>	esc_html__('Description', 'razzi-addons'),
			'additional_information' 	=>	esc_html__('Additional Information', 'razzi-addons'),
			'reviews' 					=>	esc_html__('Reviews', 'razzi-addons'),
		);
	...
	public static function edit_custom_box($column_name, $post_type) {
		...
		<fieldset class="inline-edit-col-left">
			<div class="inline-edit-col">
				<label>
					<span class="title"><?php esc_html_e('Disable', 'razzi-addons'); ?></span>
