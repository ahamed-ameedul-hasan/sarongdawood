...
class Product_Brands {
	...
	public function register_brand() {
		...
		$labels = array(
			'name'                       => __( 'Product Brands', 'razzi-addons' ),
			'singular_name'              => __( 'Brand', 'razzi-addons' ),
			'menu_name'                  => __( 'Brands', 'razzi-addons' ),
			'all_items'                  => __( 'All Brands', 'razzi-addons' ),
			'edit_item'                  => __( 'Edit Brand', 'razzi-addons' ),
			'view_item'                  => __( 'View Brand', 'razzi-addons' ),
			'update_item'                => __( 'Update Brand', 'razzi-addons' ),
			'add_new_item'               => __( 'Add New Brand', 'razzi-addons' ),
			'new_item_name'              => __( 'New Brand Name', 'razzi-addons' ),
			'parent_item'                => __( 'Parent Brand', 'razzi-addons' ),
			'parent_item_colon'          => __( 'Parent Brand:', 'razzi-addons' ),
			'search_items'               => __( 'Search Brands', 'razzi-addons' ),
			'popular_items'              => __( 'Popular Brands', 'razzi-addons' ),
			'separate_items_with_commas' => __( 'Separate brands with commas', 'razzi-addons' ),
			'add_or_remove_items'        => __( 'Add or remove brands', 'razzi-addons' ),
			'choose_from_most_used'      => __( 'Choose from the most used brands', 'razzi-addons' ),
			'not_found'                  => __( 'No brands found', 'razzi-addons' )
		);

		$permalinks         = get_option( 'product_brand_permalinks' );
		$product_brand_base = empty( $permalinks['product_brand_base'] ) ? _x( 'product-brand', 'slug', 'razzi-addons' ) : $permalinks['product_brand_base'];
	...
	public function product_brand_section( $sections ) {
		$sections['razzi_addons_product_brand'] = esc_html__( 'Product Brand', 'razzi-addons' );
	...
	public function product_brand_settings( $settings, $section ) {
		...
		$settings_product_tabs = array(
			array(
				'name' => esc_html__( 'Product Brand', 'razzi-addons' ),
				'type' => 'title',
				'id'   => $this->option . '_options',
			),
			array(
				'name'    => esc_html__( 'Product Brand', 'razzi-addons' ),
				'desc'    => esc_html__( 'Enable product brand manager', 'razzi-addons' ),
				'id'      => $this->option,
				'default' => 'yes',
				'type'    => 'checkbox',
				'checkboxgroup' => 'start',
	...
	public function settings_api_init() {
		add_settings_field(
			'product_brand_slug',
			'<label for="product_brand_slug">' . esc_html__( 'Product brand base', 'razzi-addons' ) . '</label>',
	...
	public function product_brand_slug_input() {
        ...
        <input name="product_brand_slug" type="text" class="regular-text code"
               ...
               placeholder="<?php echo esc_attr_x( 'product-brand', 'slug', 'razzi-addons' ) ?>"/>
		<?php
	}
	...
	public function add_category_fields() {
        ...
        <div class="form-field" id="product-brand-thumb-box">
            <label><?php esc_html_e( 'Thumbnail', 'razzi-addons' ); ?></label>
            ...
            <div class="product-brand-thumb-box">
                ...
                <button type="button"
                        class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'razzi-addons' ); ?></button>
                <button type="button"
                        class="remove_image_button button"><?php esc_html_e( 'Remove image', 'razzi-addons' ); ?></button>
            </div>
            <div class="clear"></div>
        </div>
		<?php
	...
	public function edit_category_fields( $term ) {
        ...
        <tr class="form-field product-brand-thumb" id="product-brand-thumb-box">
            <th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'razzi-addons' ); ?></label></th>
            <td>
                <div id="product_brand_thumb" class="product-brand-thumb"
                     data-rel="<?php echo esc_url( $this->placeholder_img_src ); ?>">
	...
	public function product_brand_columns( $columns ) {
		...
		$new_columns['thumb'] = esc_html__( 'Image', 'razzi-addons' );
	...
	public function product_brand_column( $columns, $column, $id ) {
		if ( 'thumb' == $column ) {
			...
			$columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Thumbnail', 'razzi-addons' ) . '" class="wp-post-image" height="48" width="120" />';
	...
	public function sync_product_brand_thumbnail_settings($settings, $section) {
		...
		$settings_product_tabs = array(
			...
			array(
				'name'    => esc_html__( 'Brand Thumbnails', 'razzi-addons' ),
				'desc'    => esc_html__( 'Sync the brand thumbnails', 'razzi-addons' ),
				'id'      => 'razzi_product_brand_sync_thumbnail',
				'default' => 'no',
				'type'    => 'checkbox',
			),
			array(
	...
	public function sync_product_brand_thumbnails() {
		if( get_option('razzi_product_brand_sync_thumbnail') !== 'yes' ) {
			return;
		}

		if( get_option('razzi_product_brand_sync_thumbnail_complete') === 'yes' ) {
			return;
		}
		...
		update_option('razzi_product_brand_sync_thumbnail_complete', 'yes');
	}
}
