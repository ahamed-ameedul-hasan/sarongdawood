(function ($) {
    ...
    const wdtBeforeAfterSlideWidgetHandlers = function($scope, $) {
      ...
      $.each(element_scope, function() {
        ...
        element.find(".wdt-foreground-img").css("width", `100%`);
         element.find(".wdt-foreground-img").css("clip-path", `inset(0 50% 0 0)`);

        $("#"+element_id).on("input change", (e) => {
          const BeforeAfterSlider = e.target.value;s
          element.find(".wdt-foreground-img").css("clip-path", `inset(0 calc(100% - ${BeforeAfterSlider}%) 0 0)`);
