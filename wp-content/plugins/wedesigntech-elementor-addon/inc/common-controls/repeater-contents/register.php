...
class WeDesignTech_Common_Controls_Repeater_Contents {
	...
	public function render_image_template($item, $link_start, $link_end) {
		$output = '';
		if ( ! empty( $item['media_image_template']['url'] ) ) :
			...
			$output .= '<div class="wdt-content-image-wrapper '.esc_attr($class).'">';
				$output .= '<div class="wdt-content-image">';
					...
					$media_image_template_setting = array ();
					$media_image_template_setting['image'] = $item['media_image_template'];
					$media_image_template_setting['image_size'] = 'full';
					$media_image_template_setting['image_custom_dimension'] = isset($item['media_image_template_custom_dimension']) ? $item['media_image_template_custom_dimension'] : array ();

					$output .=  ($link_start != '') ? $link_start : '<span>';
						$output .= \Elementor\Group_Control_Image_Size::get_attachment_image_html( $media_image_template_setting );
					$output .=  ($link_end != '') ? $link_end : '</span>';
