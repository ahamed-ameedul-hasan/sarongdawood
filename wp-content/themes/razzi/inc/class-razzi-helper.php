...
class Helper {
	 ...
	 * @return void
	 */
	public function blog_load_pagination() {
		global $wp_query;

		if ( 'loadmore' == $this->get_option( 'blog_nav_type' ) ) {
			$this->blog_load_more();
		} else {
			$this->blog_nav_numberic();
		}
	}
	 ...
	 * @return array
	 */
	public function blog_nav_numberic() {
		the_posts_pagination( array(
			'end_size'  => 3,
			'prev_text' => \Razzi\Icon::get_svg( 'chevron-left', '', 'ui' ),
			'next_text' => \Razzi\Icon::get_svg( 'chevron-right', '', 'ui' )
		) );
	}
	...
	public static function currency_switcher( $args = array() ) {
		...
		foreach ( $currencies as $key => $currency ) {
			if( ! empty( $currency['hide_on_front'] ) ) {
				continue;
			}
