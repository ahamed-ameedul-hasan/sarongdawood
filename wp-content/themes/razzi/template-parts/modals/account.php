...
<div class="modal-content">
	...
	<?php if ( get_option( 'woocommerce_enable_myaccount_registration' ) === 'yes' ) : ?>
        ...
        if ( $myaccount_page ) {
            $request_url = get_permalink( $myaccount_page );
        } else {
            $request_url   = remove_query_arg( '_wp_http_referer' );
        }
        ?>

        <form action="<?php echo esc_url( $request_url ); ?>" method="post" class="woocommerce woocommerce-account woocommerce-form woocommerce-form-register register"
              style="display: none;">
            ...
            <p class="form-row form-row-wide">
                <?php wp_nonce_field( 'woocommerce-register', 'woocommerce-register-nonce', false, true ); ?>
                <?php
                echo sprintf('<input type="hidden" name="_wp_http_referer" value="%s" />', esc_url($request_url));
                ?>
